      ...
      {user && interns?.length > 0 ? (
        interns.map((intern) => (
          <div key={intern.id} className="border-2 border-gray-200 rounded-lg mt-4 p-4 bg-white shadow-lg">
            <h2 className="text-2xl font-bold text-gray-800 mb-2 text-purple-600">{intern.ProjectTitle}</h2>
            <p className="text-gray-600 mb-1">
              <strong className="font-bold">Project Link:</strong> <a href={intern.ProjectLink} target="_blank" rel="noopener noreferrer">{intern.ProjectLink}</a>
            </p>
            <p className="text-gray-600 mb-1">
              <strong className="font-bold">Mentor Name:</strong> {intern.MentorName}
            </p>
            <p className="text-gray-600 mb-1">
              <strong className="font-bold">From:</strong> {intern.fromDate}
            </p>
            <p className="text-gray-600 mb-1">
              <strong className="font-bold">To:</strong> {intern.toDate}
            </p>
            <p className="text-gray-600 mb-1">
              <strong className="font-bold">Team Size:</strong> {intern.TeamSize}
            </p>
            <p className="text-gray-600 mb-1">
              <strong className="font-bold">Description:</strong> {intern.Description}
            </p>
            <p className="text-gray-600 mb-1">
              <strong className="font-bold">Organization:</strong> {intern.Organization}
            </p>
